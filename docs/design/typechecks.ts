...
// Every check in this file is a deliberate negative assertion; a missing-context report is the
// thing being asserted, not an accidental dependency omission.
...
import type { BranchName, CommitId, Repositories, RepositoriesApi, RepositoryRef, WorktreeRef } from "./repositories/api.ts";
...
export function checkRepositoryTypes(
  ...
  void repositories.inspectWorktree({ repository, branch });
  // @ts-expect-error A working-directory path is not a repository identity.
  void repositories.listWorktrees(worktree.directory);
  // @ts-expect-error Integration evidence compares pinned commits, not moving branch names.
  void repositories.assessIntegration(repository, { candidate: branch, base: commit });
...
export function checkWorkflowRequirements(input: ChangeRepositoryInput, options: WorkspaceQueryOptions): void {
  const effect: Effect.Effect<RepositoryWorkView, ChangeInspectionError, ChangeStore | Repositories> =
