...
- **formalize the split** in `docs/`: `extensions/` = contribution + wiring, `integrations/` =
...
`docs/guides/extensions.md`), but the internal names could be aligned, and one note in the docs that
...
- `review-findings-plugin-system-3.md`, `review-fixes-report.md` and
  ...
  (or fold their still-true content into `docs/guides/effect-conventions.md`, which already covers most
  of it) and delete.
