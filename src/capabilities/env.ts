/**
 * The environment every child IWE starts receives: the server's own, scrubbed of the variables
 ...
 * `env IWE_PORT=… NODE_ENV=production ELECTRON_RUN_AS_NODE=1 …`
 ...
 * `NODE_ENV=production` changes other tools' behaviour; and `IWE_PORT`/`IWE_ROOT` point a server
 ...
 * own server, and the product relies on that — the `@agent_status` writers the README documents,
 ...
 * terminal adds the change's context (`IWE_CHANGE_ID`, `IWE_CHANGE_DIR`) rather than leaving it
 * to accident.
 */
export const childEnv = (
  ...
  for (const [key, value] of Object.entries(base)) {
    ...
    if (key === "ELECTRON_RUN_AS_NODE" || key === "NODE_ENV" || key.startsWith("IWE_")) continue;
    env[key] = value;
  }
  return { ...env, ...extra };
};
