 ...
 */
/** TODO-MIGRATE — mostly synchronous filesystem work, deliberately left a Promise per the
 * migration task: an Effect wrap buys nothing here, and the one caller (integrations/git.ts)
 * keeps the old signature until the server task sweeps this. `ignored` above goes through the
 * `sh` facade for the same reason — the whole function is Promise-shaped, so wrapping one CLI
 * call in an Effect inside it would be ceremony, not structure. */
