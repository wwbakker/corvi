...
import { WidgetStateSchema } from "../api.ts";
 ...
 * `branchFor`.
 *
 * Deliberately its own section rather than living in the deployments implementation: that module
 * calls out to the `az` CLI and pulls in the rest of the backend with it, which has no business
 * in a browser bundle. A convention that both sides need to agree on cannot live somewhere only
 * one of them can import.
 */
