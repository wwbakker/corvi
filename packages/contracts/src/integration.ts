...
/** One setting an integration declares, rendered by the settings page in a section per
 * integration and stored under `extensionSettings[name][key]` — at both scopes: the global level
 * and every workspace, where the same shape overrides the global one key by key. */
export type ExtensionSetting = {
  key: string;
  label: string;
  placeholder?: string;
  hint?: string;
  /** A list of strings rather than one value, edited as rows. Works at both scopes. */
  list?: boolean;
   ...
   * still clears it. A secret is the exception to the locking rule: its `env` is a fallback
   * rather than an override, so a stored secret beats it and the page may always edit it (see
   * `apps/server/src/settings/server/secrets.ts`). */
  secret?: boolean;
  /** The environment variable that overrides this setting at every scope, shown locked when set
   * — the page cannot fight it. The precedence is the one chain everywhere: environment
   * variable, then the workspace's value, then the global one, then the default. */
  env?: string;
};
...
export type IncludedIntegration = {
  ...
  /** The settings this integration declares, rendered by the settings page in a section per
   * integration at both scopes — the global level and every workspace — and stored where the
   * integration itself reads them back: `extensionSettings[name][key]`, a workspace's overriding
   * the global one key by key. Declarations stop deciding scope: every setting may be overridden
   * per workspace, and which ones are is the user's decision. */
