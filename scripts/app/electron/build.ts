 ...
 * writes a `package.json` beside them saying where the checkout to serve is (`iweRoot`), which is
 * read at launch — the `IWERoot` of the Swift app's Info.plist, in the one place both platforms
 * already look.
 *
...
export async function buildApp(outDir: string, root: string): Promise<void> {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "iwe", version: "1.0.0", main: "main.cjs", iweRoot: root }, null, 2)}\n`,
  );
}
