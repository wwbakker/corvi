 ...
 * pinned without a renderer: an id that is not the core's dashboard or the terminals page, and
 * not a tab an extension offered, is the dashboard — a stale URL still shows something rather
 * than a blank page. "Review changes" is an extension tab now, so the URL follows the
 * extension's enablement rather than being core.
 */
...
const CORE: ReadonlySet<string> = new Set(["dashboard", "terminals"]);

/** The tabs the nav shows: the core's Dashboard, then the extensions' tabs in load order — the
 * review extension's "Review changes" among them when it is enabled. A contributed id that
 * would shadow a core page is dropped. */
