 ...
 * rewritten. The route
 * (apps/server/src/app-root/routes.ts) serves the result, falling back to index.html for the SPA's own
 * paths.
 *
...
const sourceDir = "apps/server/src/app-root";
...
export async function ensureClient(): Promise<void> {
  ...
  const newestSource = development ? await newest("apps/server/src") : 0;
