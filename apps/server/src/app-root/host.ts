 ...
 *
 * `apps/server/src/domain/host.ts` is the vocabulary both sides speak and `scripts/app/electron/preload.ts`
 ...
 * (apps/server/src/app-root/notify.tsx), and there is no window chrome to lay the page's top row out as
 * (apps/server/src/domain/chrome.ts).
 */
