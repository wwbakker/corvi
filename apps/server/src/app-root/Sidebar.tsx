...
/** The app window's first row starts with the traffic lights on macOS (apps/server/src/domain/chrome.ts), and
...
export function Sidebar({
   ...
   * tabs' job (apps/server/src/terminals/client/WindowTabs.tsx): the column is for going to the ones there
  ...
  return (
    <nav className="sidebar" style={{ width }}>
      {/* The window's own top row: on macOS the traffic lights sit here, and in the app window it
          is what you drag the window by (apps/server/src/domain/chrome.ts). The switcher moves up into it so
