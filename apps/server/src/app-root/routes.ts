...
export const appRootRoutes = guard({
  ...
  "/icons/:file": async (req) => {
    ...
    const response = await fileResponse(join("apps/server/src/app-root/icons", basename(req.params.file)), {
  ...
  // change (development) by apps/server/src/app-root/client.ts.
