...
function App(): JSX.Element {
  ...
  // it. A draft is not a change — nothing is written until "Create idea" (apps/server/src/wizard/draft.ts).
  ...
  // The right-click menu is the host's to draw and the setting's to decide (apps/server/src/app-root/contextMenu.ts).
  ...
  // traffic lights macOS keeps in it (apps/server/src/domain/chrome.ts). A browser has neither, so the row is
      ...
      <main className={onTerminal ? "content flush" : "content"}>
        ...
        {view.name === "change" && (
          <ChangeView
            ...
            // came back, with nothing left to read them again (apps/server/src/change-page/client/ChangeView.tsx).
            key={view.id}
            id={view.id}
            page={view.page}
            platform={platform}
